...
function flash(color) {
  ...
  setTimeout(() => pad.classList.remove("lit"), 1000);
  /* Increased the panel flash time */
}
...
function playSequence() {
  ...
  sequence.forEach((clr, i) => {
    ...
    setTimeout(() => flash(clr), i * 1000);
    /* increased the time in between flashes */
  });
}
...
function handleClick(chosen) {
  if (chosen === sequence[playerPos]) {
    ...
    alert("It's finally over, you're free. Final score: " + sequence.length * 47658);
    resetGame();
  }
}
